...
delete FROM produtos WHERE TAMANHO = "1 LITRO"                 # Deletando mais de de uma. 
AND	substring(DESCRITOR, 1, 5) = "Sabor dos Alpes";

select codigo from produtos where 
CODIGO not in ( select codigo_do_produto from sucos_vendas.tabela_de_produtos);    # Selecionando os produtos que existem na tabela de produtos mais nao existem na tabela de codigo_do_produto. 

delete from produtos where
CODIGO not in ( select codigo_do_produto from sucos_vendas.tabela_de_produtos);   # Deletando para deixa as duas tabelas sincronizadas. 
